...
const adminhome = () => {
  ...
  const handleContentChange = (fieldName, newContent, isEditing = false) => {
    if (isEditing) {
      setEditingProject((prev) => {
        // Handle nested blogContent fields
        if (fieldName.startsWith('blogContent.')) {
  ...
  const handleAddProject = async () => {
    ...
    try {
      ...
      const hasBlogContent = Object.values(newProject.blogContent).some(value => value.trim() !== '');
      if (!hasBlogContent) {
        throw new Error('Blog content is required');
      }

      // First, create the main project document
      ...
      const blogContentDoc = await addDoc(blogContentRef, {
        components: newProject.blogContent.components || "",
        circuit: newProject.blogContent.circuit || "",
        about_circuit: newProject.blogContent.about_circuit || "",
        about_programming: newProject.blogContent.about_programming || "",
        conclusion: newProject.blogContent.conclusion || "",
        conclusion_images: newProject.blogContent.conclusion_images || "",
        programming: newProject.blogContent.programming || "",
        resources: newProject.blogContent.resources || "",
      });

      // Update the local state with the new project including blog content
      const newProjectWithId = {
        ...newProject,
        id: projectDocRef.id,
        blogContent: {
          id: blogContentDoc.id,
          ...newProject.blogContent
        }
      };
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <Tabs defaultValue="addproject" className="mb-8 items-center">
        <div className="navbar bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-6 pt-12 pb-2 flex flex-row h-fit fixed left-1/2 transform -translate-x-1/2 -translate-y-1/2  w-full z-[6000]">
          <h1 className="text-2xl font-semibold text-center">Qnagenius Admin</h1>
          <TabsList className="flex px-4 py-6 w-fit mx-auto">
            <TabsTrigger value="addproject">Projects</TabsTrigger>
        ...
        <div className="pt-14">
          ...
          <TabsContent value="addproject">
            <div className="container mx-auto p-4 bg-white shadow-sm border-[1px] border-gray-300 rounded-xl">
              <Tabs defaultValue="add" className="mb-8">
                <TabsList>
                  <TabsTrigger value="add">New Project</TabsTrigger>
                            ...
                            <Tabs defaultValue="content" className="mb-8">
                              ...
                              <TabsContent value="files">
                                <Card>
                                  ...
                                  <CardContent>
                                    <div className={`flex flex-row gap-2 px-4 py-2 w-fit rounded-sm cursor-pointer`}>
